...
**Implementation note** 
Compute residuals conditional on initial zeros or observed history, then apply Gaussian log-likelihood over f(y_t | F_{t-1}; \theta).
...
**Implementation note** 
Iteratively forecast future steps using the fitted seasonal and non-seasonal structure for h=1,\ldots,H.
...
```c
void aa_train_test_split(vars Series,int N,var Train Ratio,vars Train,vars Test,int* TrainN,int* TestN)
```
...
| `Series` | `vars` | $y_t$ | Primary series argument supplied to the routine. |
| `N` | `int` | $N$ | Sample length or active number of observations. |
| `Train Ratio` | `var` | $Train Ratio$ | Routine-specific input or output argument. |
